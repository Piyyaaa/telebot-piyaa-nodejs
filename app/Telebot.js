...
class Telebot extends TelegramBot {
    ...
    getGreeting() {
        this.onText(commands.greeting, (data) => {
            this.sendMessage(data.from.id, "Halo juga kamu!")
        })
    }
    getFollow() {
    ...
    getQuotes() {
        this.onText(commands.quote, async(data) => {
            ...
            try {
                ...
                console.error(error)
                this.sendMessage9data.from.id, "Maaf silahkan ulangi lagi"
            }
        })
    }
    getNews() {
    ...
    getQuake() {
        ...
        this.sendMessage(data.from.id, "Mohon tunggu sebentar..")
            try {
                ...
                this.sendPhoto(data.from.id, imgSourceUrl, {caption: ` Info gempa terbaru ${Tanggal} / ${Jam}.\n\nWilayah: ${Wilayah}\nBesaran ${Magnitude} SR\nKedalaman : ${Kedalaman}`})
            } catch (error) {
                console.error (error)
            }
        })
